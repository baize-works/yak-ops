...
class SeaTunnelEngineGatewayTest extends EngineAdapterContract {
    @Override protected Harness harness() {
        Stub client=new Stub(); SeaTunnelEngineGateway gateway=new SeaTunnelEngineGateway(client);
        EngineEndpoint endpoint=new EngineEndpoint(new ExecutionEngine("seatunnel"),"1","http://engine",null,Map.of());
        return new Harness() {
            public EngineGateway gateway(){return gateway;} public EngineEndpoint endpoint(){return endpoint;}
            public JobSubmitCommand command(String key){return new JobSubmitCommand(endpoint,"env {}",Map.of("fileName","job.conf"),key);}
    ...
    static class Stub implements SeaTunnelEngineClient {
        ...
        public SeaTunnelJobResponse job(long id,String job){if(timeout)throw new IllegalStateException("read timed out");return new SeaTunnelJobResponse(status,null,List.of(),List.of());}
        public SeaTunnelMetricsResponse metrics(long id,String job){return new SeaTunnelMetricsResponse(Map.of("records",9));}
