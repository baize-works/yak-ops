...
-- 2. The server-id uses the platform-recommended default value of 5600; LinkUp generates one randomly when it is not configured.
...
('connector', 'MySQL-CDC', 'source', 'format', 'MySQL CDC 输出格式，可选 DEFAULT 或 COMPATIBLE_DEBEZIUM_JSON。', 'enum', 0, 'DEFAULT', 'COMPATIBLE_DEBEZIUM_JSON', '{"summary":"控制 MySQL CDC 事件输出的数据格式。","coreMeaning":"DEFAULT 使用 LinkUp 内部 CDC 事件结构；COMPATIBLE_DEBEZIUM_JSON 输出兼容 Debezium 的 JSON。","validValues":["DEFAULT","COMPATIBLE_DEBEZIUM_JSON"],"recommendationHints":["LinkUp 内部链路通常使用 DEFAULT。","需要兼容 Debezium 消费格式时使用 COMPATIBLE_DEBEZIUM_JSON。"],"cautions":["切换格式会改变下游字段结构和解析逻辑。"]}', '用于AI参数推荐', 0),
...
('connector', 'MySQL-CDC', 'source', 'common-options', 'Source 插件通用参数集合，具体字段参见 LinkUp Source Common Options。', 'object', 0, NULL, '{"plugin_output":"mysql_cdc_result","parallelism":4}', '{"summary":"承载 Source 插件公共配置。","coreMeaning":"通常包括结果表名、并行度等非 MySQL CDC 专属参数。","recommendationHints":["仅配置当前 LinkUp 版本明确支持的公共参数。"],"cautions":["该项更适合作为参数组展示，而非直接输出 common-options 键。"]}', '用于AI参数推荐', 0)
AS incoming
