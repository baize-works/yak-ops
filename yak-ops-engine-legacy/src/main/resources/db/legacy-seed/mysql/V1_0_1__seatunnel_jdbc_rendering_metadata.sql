...
-- Note: Parameter names follow the LinkUp configuration options provided by the user, such as fetch_size and split.size.
...
('connector', 'Jdbc', 'source', 'dialect', '显式指定 JDBC 方言，优先级高于通过 URL 自动识别。', 'string', 0, NULL, 'mysql', '{"summary":"指定 LinkUp 使用的 JDBC 方言。","coreMeaning":"方言决定类型映射、SQL 生成、元数据读取等数据库差异化行为。","recommendationHints":["URL 无法准确识别或使用兼容数据库时显式配置。"],"cautions":["错误方言可能导致类型映射或 SQL 语法错误。","未支持方言可能回退到 GenericDialect。"]}', '用于AI参数推荐', 0),
...
('connector', 'Jdbc', 'source', 'partition_column', '用于并行分片读取的列名。', 'string', 0, NULL, 'id', '{"summary":"指定 query 模式下的分片列。","coreMeaning":"LinkUp 根据该列的上下界划分多个读取分区。","recommendationHints":["优先选择有索引、分布较均匀、可比较的数值列。"],"cautions":["分布严重倾斜会造成任务长尾。","该参数主要用于 query 模式。"]}', '用于AI参数推荐', 0),
('connector', 'Jdbc', 'source', 'partition_upper_bound', '分片列扫描上界；未配置时 LinkUp 会查询最大值。', 'number', 0, NULL, '1000000', '{"summary":"设置 partition_column 的最大扫描边界。","coreMeaning":"与下界及分片数共同决定 query 模式的分区范围。","recommendationHints":["边界已知且稳定时可显式配置，减少额外元数据查询。"],"cautions":["配置过小会漏读超过上界的数据。","应与 partition_column 数据类型匹配。"]}', '用于AI参数推荐', 0),
('connector', 'Jdbc', 'source', 'partition_lower_bound', '分片列扫描下界；未配置时 LinkUp 会查询最小值。', 'number', 0, NULL, '1', '{"summary":"设置 partition_column 的最小扫描边界。","coreMeaning":"与上界及分片数共同决定 query 模式的分区范围。","recommendationHints":["边界已知且稳定时可显式配置。"],"cautions":["配置过大会漏读低于下界的数据。","应与 partition_column 数据类型匹配。"]}', '用于AI参数推荐', 0),
...
('connector', 'Jdbc', 'source', 'common-options', 'Source 插件通用参数集合，具体字段参见 LinkUp Source Common Options。', 'object', 0, NULL, '{"plugin_output":"jdbc_source_result"}', '{"summary":"承载 Source 插件公共能力参数。","coreMeaning":"不是 JDBC 专属业务参数，通常用于结果表名、并行度等公共配置。","recommendationHints":["仅填写当前 LinkUp 版本明确支持的公共参数。"],"cautions":["不同版本支持项可能不同。","该项可作为参数组展示，而非直接输出 common-options 键。"]}', '用于AI参数推荐', 0),
...
('connector', 'Jdbc', 'sink', 'primary_keys', '目标表主键字段列表，用于自动生成 INSERT、UPDATE、DELETE 或 UPSERT SQL。', 'array', 0, NULL, '["id"]', '{"summary":"声明目标表主键。","coreMeaning":"LinkUp 使用主键识别记录并生成 Upsert、更新和删除语义。","recommendationHints":["CDC 或需要幂等写入时应准确配置。","联合主键按字段顺序填写。"],"cautions":["错误主键可能导致重复数据或误更新。","目标表应具备对应唯一性约束。"]}', '用于AI参数推荐', 0),
...
('connector', 'Jdbc', 'sink', 'generate_sink_sql', '是否基于目标数据库表结构自动生成 Sink SQL。', 'boolean', 0, 'false', 'true', '{"summary":"控制是否自动生成 JDBC Sink 写入 SQL。","coreMeaning":"开启后 LinkUp 根据目标表、字段和主键生成语句。","recommendationHints":["标准表结构和字段映射场景可开启。"],"cautions":["复杂表达式、特殊函数或非标准写法应使用 query。"]}', '用于AI参数推荐', 0),
('connector', 'Jdbc', 'sink', 'xa_data_source_class_name', 'XA 数据源类名，例如 MySQL 使用 com.mysql.cj.jdbc.MysqlXADataSource。', 'string', 0, NULL, 'com.mysql.cj.jdbc.MysqlXADataSource', '{"summary":"指定 Exactly-Once 使用的 XADataSource 实现类。","coreMeaning":"LinkUp 通过该类创建 XA 连接并参与两阶段提交。","recommendationHints":["仅在 is_exactly_once=true 时配置。"],"cautions":["类名必须与数据库驱动版本匹配。","普通 DataSource 类不能替代 XADataSource。"]}', '用于AI参数推荐', 0),
...
('connector', 'Jdbc', 'sink', 'common-options', 'Sink 插件通用参数集合，具体字段参见 LinkUp Sink Common Options。', 'object', 0, NULL, '{"parallelism":4}', '{"summary":"承载 Sink 插件公共能力参数。","coreMeaning":"不是 JDBC 专属参数，通常用于并行度等通用任务配置。","recommendationHints":["仅填写当前 LinkUp 版本支持的公共参数。"],"cautions":["该项可作为参数组展示，而不是直接输出 common-options 键。"]}', '用于AI参数推荐', 0),
