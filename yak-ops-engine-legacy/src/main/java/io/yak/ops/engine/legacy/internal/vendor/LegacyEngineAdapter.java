...
/** LinkUp REST adapter. Maps vendor responses before they leave this module. */
...
public class LegacyEngineAdapter implements EngineGateway, EngineMetricsGateway {
    private final LinkUpEngineClient client;
    ...
    public LegacyEngineAdapter(LegacyRestClient client) { this(new RestTemplateLinkUpEngineClient(client)); }
    LegacyEngineAdapter(LinkUpEngineClient client) { this.client = client; }
    ...
    @Override public synchronized JobExecution submit(JobSubmitCommand command) {
        ...
        try {
            ...
            LinkUpSubmitResponse response = client.submit(clientId(command.endpoint()), command.definition().getBytes(StandardCharsets.UTF_8), file);
            if (response == null || response.jobId() == null || response.jobId().trim().isEmpty())
                throw new EngineSubmissionException("LinkUp submit response does not contain an execution id", java.util.Collections.unmodifiableMap(new java.util.LinkedHashMap() {{ put("vendor", "linkup"); put("vendor.error_code", "INVALID_RESPONSE"); }}));
        ...
        } catch (EngineContractException e) { throw e; } catch (Exception e) { throw LinkUpErrorMapper.submission(e); }
    }
    @Override public void cancel(EngineEndpoint endpoint, String jobId) { try { client.cancel(clientId(endpoint), jobId); } catch (Exception e) { throw LinkUpErrorMapper.unavailable("cancel", e); } }
    @Override public JobExecution execution(EngineEndpoint endpoint, String platformId, String jobId) {
        try { LinkUpJobResponse info = client.job(clientId(endpoint), jobId); if (info == null) throw LinkUpErrorMapper.unavailable("query", new IllegalStateException("empty response"));
            LinkUpExecutionStatusMapper.StatusResolution status = LinkUpExecutionStatusMapper.resolve(info.status());
        ...
        catch (EngineContractException e) { throw e; } catch (Exception e) { throw LinkUpErrorMapper.unavailable("query", e); }
    }
    @Override public EngineMetrics metrics(EngineEndpoint endpoint, String jobId) { capabilities().require(EngineCapabilities.Capability.METRICS); try { LinkUpMetricsResponse response = client.metrics(clientId(endpoint), jobId); if (response == null) throw LinkUpErrorMapper.unavailable("metrics", new IllegalStateException("empty response")); return LinkUpMetricsMapper.map(response.values()); } catch (EngineContractException e) { throw e; } catch (Exception e) { throw LinkUpErrorMapper.unavailable("metrics", e); } }
