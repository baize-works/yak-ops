      ...
      <div className="grid grid-cols-2 items-start gap-5 max-lg:grid-cols-1">
        <EndpointPanel icon={<DatabaseOutlined />} title="Source 来源配置">
          <div style={{ height: 320, overflow: "auto" }}>
            ...
            <Button style={{ width: "100%", marginTop: 12 }}>数据预览</Button>
          </div>
        </EndpointPanel>
            ...
            {sinkConfig.autoCreateTable ? (
              <div>
                ...
                <Input
                  ...
                  onChange={(event: ChangeEvent<HTMLInputElement>) =>
                    onSinkChange({ targetTableName: event.target.value })
                  }
                />
              </div>
            ) : (
              <div>
                ...
                <Select
                  ...
                  placeholder={
                    targetReady ? "请选择目标表" : "请先选择目标数据源"
                  }
                  optionFilterProp="label"
                  className="w-full"
                  onChange={(table: string) =>
                    onSinkChange({ table, primaryKey: "" })
                  }
                />
              </div>
            )}
