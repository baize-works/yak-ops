...
const NODE_TEMPLATES: NodeTemplate[] = [
  { type: 'TASK', label: '任务节点', description: '通用任务执行节点', icon: <CirclePlay size={16} /> },
  { type: 'DATA', label: '数据处理', description: '模拟数据处理任务', icon: <Database size={16} /> },
  { type: 'CHECK', label: '校验节点', description: '模拟规则或质量校验', icon: <ShieldCheck size={16} /> },
  { type: 'NOTICE', label: '通知节点', description: '模拟消息通知任务', icon: <Bell size={16} /> },
];
...
const nodeStateClass = (status?: string, selected?: boolean) => {
  ...
  if (status === 'PAUSING' || status === 'PAUSED') return 'border-[#b8bdc5] bg-[#f6f7f8] shadow-none';
  if (status === 'FAILED' || status === 'TIMED_OUT') return 'border-[#d92d20] bg-[#fff6f4] shadow-[0_0_0_2px_rgba(217,45,32,.06)]';
  if (status === 'UPSTREAM_FAILED') return 'border-[#cfd3d8] bg-[#f5f6f7] opacity-70 shadow-none';
  if (status === 'SUCCESS') return 'border-[rgba(22,24,35,.45)] bg-[#f7f7f8]';
  if (status === 'SUBMITTED' || status === 'READY') return 'border-[#aeb2ba] bg-[#fafafa]';
  if (selected) return 'border-[#fe2c55] bg-white';
  return 'border-[#dfe1e5] bg-white';
};

const statusTextClass = (status?: string) => {
  if (status === 'RUNNING' || status === 'RESUMING') return 'font-medium text-[#fe2c55]';
  if (status === 'FAILED' || status === 'TIMED_OUT') return 'font-medium text-[#d92d20]';
  if (status === 'UPSTREAM_FAILED' || status === 'PAUSED' || status === 'PAUSING') return 'font-medium text-[rgba(22,24,35,.48)]';
  ...
  <div className={[
    ...
    <div className="flex items-center justify-between gap-3">
      <div className="flex items-center gap-1.5">
        ...
        {data.mockResult === 'FAILED' ? <span className="rounded bg-[#fff0f0] px-1.5 py-0.5 text-[9px] font-medium text-[#d92d20]">模拟失败</span> : null}
        {data.continuedAfterFailure ? <span className="rounded bg-[#f0f1f2] px-1.5 py-0.5 text-[9px] font-medium text-[rgba(22,24,35,.58)]">已放行</span> : null}
      </div>
      {data.executionStatus ? (
        <span className="flex items-center gap-1 text-[10px]">
          {statusIcon(data.executionStatus)}
          <span className={statusTextClass(data.executionStatus)}>{statusLabel[data.executionStatus] || data.executionStatus}</span>
        </span>
      ) : null}
    </div>
...
const WorkflowDefinitionContent = () => {
  ...
  const applySnapshot = (snapshot: WorkflowInstance) => {
    ...
    setNodes((current) => current.map((node) => ({
      ...
      data: {
        ...
        continuedAfterFailure: continuedFailureNodeIdsRef.current.has(node.id) || node.data.continuedAfterFailure,
      },
    })));
    setEdges((current) => current.map((edge) => {
      ...
      if (blockedPath) { stroke = '#c8ccd3'; strokeDasharray = '5 5'; }
      else if (targetRunning) { stroke = '#fe2c55'; strokeWidth = 2.2; }
      else if (failedPath) { stroke = '#d92d20'; strokeWidth = 1.8; }
      else if (completedPath) { stroke = '#555b66'; strokeWidth = 1.8; }
      else if (readyPath) { stroke = '#8a8f99'; strokeWidth = 1.7; }
      return { ...edge, animated: targetRunning, style: { ...edge.style, stroke, strokeWidth, strokeDasharray } };
    }));
  };

  const handleConnect = (connection: Connection) => {
    setEdges((current) => addEdge({ ...connection, type: 'smoothstep', style: { stroke: '#d9dce1', strokeWidth: 1.4 } }, current));
  };

  const handleDragStart = (event: DragEvent<HTMLDivElement>, template: NodeTemplate) => {
    event.dataTransfer.setData('application/yak-workflow-node', JSON.stringify({ type: template.type, label: template.label }));
  ...
  const resetExecutionVisuals = () => {
    ...
    setNodes((current) => current.map((node) => ({ ...node, data: { ...node.data, executionStatus: 'WAITING', continuedAfterFailure: false } })));
    setEdges((current) => current.map((edge) => ({ ...edge, animated: false, style: { ...edge.style, stroke: '#d9dce1', strokeWidth: 1.4, strokeDasharray: undefined } })));
  };

  const handleRun = async () => {
    if (!nodes.length) { message.warning('请先拖入至少一个节点'); return; }
    ...
    try {
      ...
      closeStreamRef.current?.(); closeStreamRef.current = null;
      message.error(error instanceof Error ? error.message : '工作流运行失败');
    } finally { setSubmitting(false); }
  };

  const executeControl = async (action: string, handler: (id: string) => Promise<WorkflowInstance>, successMessage: string) => {
    if (!activeInstance || controlAction) return;
    setControlAction(action);
    try { applySnapshot(await handler(activeInstance.id)); message.success(successMessage); }
    catch (error) { message.error(error instanceof Error ? error.message : '工作流操作失败'); }
    finally { setControlAction(undefined); }
  };

  const restartStreamIfNeeded = (executionId: string, previousStatus: string, snapshot: WorkflowInstance) => {
  ...
  const handleRetryFailedNode = async (nodeId: string) => {
    ...
    setRecoveringNodeId(nodeId); setContextMenu(undefined);
    try {
      const retried = await retryWorkflowFailedNode(executionId, nodeId);
      continuedFailureNodeIdsRef.current.delete(nodeId);
      applySnapshot(retried); restartStreamIfNeeded(executionId, previousStatus, retried);
      message.success('已重新执行当前失败节点');
    } catch (error) { message.error(error instanceof Error ? error.message : '重新执行当前节点失败'); }
    finally { setRecoveringNodeId(undefined); }
  };
  ...
  const handleContinueAfterFailure = async (nodeId: string) => {
    ...
    try {
      ...
      applySnapshot(continued); restartStreamIfNeeded(executionId, previousStatus, continued);
      message.success('已保留当前失败并继续后续节点');
    } catch (error) { message.error(error instanceof Error ? error.message : '继续执行失败'); }
    finally { setRecoveringNodeId(undefined); }
  };

  const clearCanvas = () => {
    closeStreamRef.current?.(); closeStreamRef.current = null;
    continuedFailureNodeIdsRef.current.clear(); setContextMenu(undefined); setActiveInstance(undefined); setNodes([]); setEdges([]);
  };
    ...
    <div className="w-[380px] space-y-3">
      ...
      <div>
        ...
        <div className="mt-1 flex items-center gap-2">
          <InputNumber min={0} value={workflowTimeoutSeconds} onChange={(value) => setWorkflowTimeoutSeconds(Number(value || 0))} disabled={workflowLocked} className="w-[150px]" />
      ...
      <div>
        ...
        <Input.TextArea rows={6} value={workflowInputText} onChange={(event) => setWorkflowInputText(event.target.value)} disabled={workflowLocked} className="mt-1 font-mono !text-[11px]" placeholder={'{\n  "requestId": "REQ-001"\n}'} />
      </div>
    </div>
  );
    ...
    <div className="flex h-[calc(100vh-48px)] min-h-[620px] overflow-hidden bg-white">
      <aside className="w-[232px] shrink-0 border-r border-[#e8e9ec] bg-[#fafafa]">
        <div className="border-b border-[#e8e9ec] px-4 py-3.5">
          ...
          <div className="mt-1 text-xs leading-5 text-[rgba(22,24,35,.48)]">拖拽到右侧画布后连接节点</div>
        </div>
        <div className="space-y-2 p-3">
          {NODE_TEMPLATES.map((template) => (
            <div key={template.type} draggable={!workflowLocked} onDragStart={(event) => handleDragStart(event, template)} className={[
              ...
              <div className="flex items-center gap-2 text-[13px] font-semibold text-[#161823]">
                <span className="flex h-7 w-7 items-center justify-center rounded-md bg-[#f4f4f5] text-[rgba(22,24,35,.66)]">{template.icon}</span>
                {template.label}
              </div>
              <div className="mt-1.5 pl-9 text-[11px] text-[rgba(22,24,35,.45)]">{template.description}</div>
            </div>
          ))}
        </div>
      </aside>
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        <div className="flex h-12 shrink-0 items-center justify-between border-b border-[#e8e9ec] px-4">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <Input value={workflowName} onChange={(event) => setWorkflowName(event.target.value)} variant="filled" className="w-[220px]" disabled={workflowLocked} />
            <Popover content={runtimeConfig} title="运行配置" trigger="click"><Button size="small" icon={<Settings2 size={13} />}>运行配置</Button></Popover>
            ...
            {activeInstance ? (
              <div className="flex items-center gap-2 border-l border-[#ececef] pl-3 text-[11px]">
                <span className="flex items-center gap-1.5 text-[rgba(22,24,35,.62)]">{statusIcon(activeInstance.status)}{statusLabel[activeInstance.status] || activeInstance.status}</span>
          ...
          <div className="flex items-center gap-2">
            {activeInstance?.status === 'RUNNING' ? <Button icon={<Pause size={14} />} loading={controlAction === 'pause'} onClick={() => void executeControl('pause', pauseWorkflowInstance, '已请求暂停工作流')}>暂停</Button> : null}
            {activeInstance?.status === 'PAUSED' ? <Button icon={<Play size={14} />} loading={controlAction === 'resume'} onClick={() => void executeControl('resume', resumeWorkflowInstance, '已请求恢复工作流')}>恢复</Button> : null}
            ...
            <Popconfirm title="清空当前画布？" okText="清空" cancelText="取消" disabled={workflowLocked} onConfirm={clearCanvas}>
              <Button icon={<RotateCcw size={14} />} disabled={workflowLocked}>清空</Button>
            </Popconfirm>
            <Button type="primary" icon={<Play size={14} />} loading={submitting} disabled={workflowLocked} onClick={handleRun}>运行</Button>
          </div>
        </div>

        <div ref={wrapperRef} className="relative min-h-0 flex-1" onDrop={handleDrop}>
          {selectedNode && !workflowLocked ? (
            <div className="absolute right-4 top-4 z-20 w-[320px] rounded-lg border border-[#e3e5e8] bg-white p-3 shadow-md">
              <div className="text-[12px] font-semibold text-[#161823]">节点配置</div>
              <div className="mt-1 truncate text-[11px] text-[rgba(22,24,35,.48)]">{selectedNode.data.label}</div>
              <div className="mt-3 grid grid-cols-2 gap-2">
                <div>
                  <div className="text-[10px] text-[rgba(22,24,35,.48)]">触发规则</div>
                  <Select className="mt-1 w-full" size="small" value={selectedNode.data.triggerRule} options={TRIGGER_RULE_OPTIONS} onChange={(value) => updateSelectedNode({ triggerRule: value as WorkflowTriggerRule })} />
                </div>
                <div>
                  <div className="text-[10px] text-[rgba(22,24,35,.48)]">失败策略</div>
                  <Select className="mt-1 w-full" size="small" value={selectedNode.data.failurePolicy} options={NODE_FAILURE_OPTIONS} onChange={(value) => updateSelectedNode({ failurePolicy: value as WorkflowNodeFailurePolicy })} />
                </div>
              </div>
              <div className="mt-3 text-[11px] font-medium text-[rgba(22,24,35,.62)]">模拟结果</div>
              <Segmented block size="small" className="mt-1.5" value={selectedNode.data.mockResult} options={[{ label: '正常成功', value: 'SUCCESS' }, { label: '模拟失败', value: 'FAILED' }]} onChange={(value) => updateSelectedNode({ mockResult: value as WorkflowMockResult })} />
              <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <Input.TextArea rows={4} className="mt-1 font-mono !text-[10px]" value={selectedNode.data.inputMappingText} onChange={(event) => updateSelectedNode({ inputMappingText: event.target.value })} placeholder={'{\n  "requestId": "$workflow.requestId",\n  "value": "upstream.value"\n}'} />
              <div className="mt-1.5 text-[9px] leading-4 text-[rgba(22,24,35,.38)]">超时填 0 表示关闭；Input Mapping 只能引用 Workflow Input 或直接前置节点 Output。</div>
            </div>
          ) : null}

          {contextMenu ? (
            <div className="absolute z-30 w-[248px] rounded-lg border border-[#e1e4e8] bg-white p-1.5 shadow-lg" style={{ left: contextMenu.x, top: contextMenu.y }} onMouseDown={(event) => event.stopPropagation()}>
              <Button type="text" block icon={<RefreshCw size={14} />} loading={recoveringNodeId === contextMenu.nodeId} className="!flex !h-8 !items-center !justify-start !px-2 !text-[12px]" onClick={() => void handleRetryFailedNode(contextMenu.nodeId)}>重新执行当前节点</Button>
              <div className="px-2 pb-1 text-[10px] leading-4 text-[rgba(22,24,35,.42)]">当前节点重新执行，成功后再继续后续节点。</div>
              <div className="mx-2 my-1 border-t border-[#f0f0f1]" />
              <Button type="text" block icon={<ArrowRightCircle size={14} />} disabled={Boolean(recoveringNodeId)} className="!flex !h-8 !items-center !justify-start !px-2 !text-[12px]" onClick={() => void handleContinueAfterFailure(contextMenu.nodeId)}>跳过当前失败，继续后续节点</Button>
              <div className="px-2 pb-1 text-[10px] leading-4 text-[rgba(22,24,35,.42)]">保留当前失败结果，直接解除受它影响的后继阻断。</div>
            </div>
          ) : null}
          ...
          <ReactFlow
            ...
            onNodeContextMenu={(event, node) => {
              ...
              if (!activeInstance || data.executionStatus !== 'FAILED' || data.continuedAfterFailure || !wrapperRef.current) { setContextMenu(undefined); return; }
              const bounds = wrapperRef.current.getBoundingClientRect();
              setContextMenu({ nodeId: node.id, x: Math.max(8, Math.min(event.clientX - bounds.left, bounds.width - 256)), y: Math.max(8, Math.min(event.clientY - bounds.top, bounds.height - 150)) });
            }}
            onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = workflowLocked ? 'none' : 'move'; }}
...
const WorkflowDefinitionPage = () => <ReactFlowProvider><WorkflowDefinitionContent /></ReactFlowProvider>;
