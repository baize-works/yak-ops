...
const FOLDER_GROUPS: Array<{
  ...
  { id: 'integration', label: '数据集成任务' },
  { id: 'resource', label: '资源文件' },
];

const RIGHT_PANEL_ITEMS: RightPanelItem[] = [
  { key: 'properties', label: '属性', icon: FileJson2 },
  { key: 'run', label: '运行配置', icon: SlidersHorizontal },
  { key: 'schedule', label: '调度配置', icon: Clock3 },
  { key: 'version', label: '版本', icon: GitBranch },
];
...
const createFile = (
  ...
  kind: WorkspaceFileKind,
  content: string,
  options: Partial<WorkspaceFile> = {},
): WorkspaceFile => ({
  id,
  name,
  kind,
  folder: kind,
  engine: FILE_KIND_META[kind].engine,
  content,
  owner: 'me',
...
const INITIAL_FILES: WorkspaceFile[] = [
  createFile('mysql-sql-etl', 'mysql_sql_etl.sql', 'sql', DEFAULT_SQL, {
    engine: 'Flink SQL',
    favorite: true,
  ...
  createFile(
    'user-behavior-agg',
    'user_behavior_agg.sql',
    'sql',
    joinLines(
      '-- 用户行为指标按天聚合',
      'INSERT OVERWRITE TABLE ads_user_behavior_daily',
      'SELECT user_id, COUNT(*) AS event_count, dt',
      'FROM dwd_user_behavior',
      'WHERE dt = ${bizdate}',
      'GROUP BY user_id, dt;',
    ),
    { engine: 'Spark SQL', updatedAt: '2026-08-04 08:46' },
  ),
  createFile(
    'dim-user-profile',
    'dim_user_profile.sql',
    'sql',
    joinLines(
      '-- 用户画像维度表',
    ...
    { engine: 'Hive', owner: 'other', favorite: true },
  ),
  createFile(
    'ods-to-dwd-flink',
    'ods_to_dwd_flink.sql',
    'flink',
    joinLines(
      '-- ODS 实时清洗到 DWD',
  ...
  createFile(
    ...
    { favorite: true, updatedAt: '2026-08-03 16:21' },
  ),
  createFile(
    'etl-user-profile',
    'etl_user_profile.py',
  ...
  createFile(
    ...
    { favorite: true, updatedAt: '2026-08-03 18:45' },
  ),
  createFile(
    'sync-mysql-odps',
    'sync_mysql_to_odps_20260801',
    'integration',
    joinLines(
      '{',
...
const DataDevelopmentWorkbench = () => {
  ...
  const [expandedFolders, setExpandedFolders] = useState<
    Record<WorkspaceFileKind, boolean>
  >({
    sql: true,
    flink: true,
    python: true,
  ...
  const updateActiveContent = useCallback(
    (content: string) => {
      ...
      setFiles((current) =>
        current.map((file) =>
          file.id === activeFileId && file.content !== content
  ...
  const openCreateModal = (kind: WorkspaceFileKind = 'sql') => {
    form.setFieldsValue({ kind, folder: kind, name: '' });
    setCreateOpen(true);
  };
  ...
  const deleteActiveFile = () => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        setFiles((current) =>
          current.filter((file) => file.id !== activeFile.id),
        );
  ...
  const renderFileRow = (file: WorkspaceFile) => {
    ...
    return (
      ...
      >
        <Icon
          size={14}
          className={selected ? undefined : meta.iconClassName}
        />
            ...
            <div className="space-y-4">
              ...
              <PanelField label="节点类型">
                <Input
                  ...
                  disabled
                  value={
                    activeFile ? FILE_KIND_META[activeFile.kind].label : ''
                  }
                />
              </PanelField>
              <PanelField label="计算引擎">
                <Select
                  ...
                  onChange={(engine) =>
                    activeFile &&
                    updateFile(activeFile.id, { engine, dirty: true })
                  }
                />
              </PanelField>
              <PanelField label="描述">
          ...
          {rightPanel === 'version' && (
            <div className="space-y-1">
              {[
                ...
                {
                  version: 'v0.7',
                  description: '调整字段映射与并行度',
                  time: '08-04 08:36',
                  current: false,
                },
                {
                  version: 'v0.6',
                  description: '增加用户行为清洗逻辑',
                  time: '08-03 19:20',
                  current: false,
                },
                {
                ...
                <div className="flex items-center justify-between gap-2">
                  <Select
                    ...
                    options={[
                      {
                        label: '用户数据平台',
                        value: 'user-data-platform',
                      },
                      {
                        label: '实时数仓项目',
                        value: 'realtime-warehouse',
                      },
                    ]}
                  />
                ...
                <div className="mt-2 flex items-center gap-1">
                  {[
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setExplorerFilter(value as ExplorerFilter)
                      }
                      className={[
                  ...
                  <Dropdown
                    ...
                    menu={{
                      ...
                      onClick: ({ key }) =>
                        openCreateModal(key as WorkspaceFileKind),
                    }}
                  >
                    <Button
                      type="primary"
                ...
                <div className="flex h-full items-center justify-center bg-[#fbfbfc] p-8">
                  <div className="w-full max-w-[620px] text-center">
                    ...
                    <p className="m-0 text-[13px] text-[rgba(22,24,35,0.48)]">
                      从左侧打开开发节点，或新建 SQL、Python、Shell 和
                      Notebook 任务。
                    </p>
                    <Button
                      type="primary"
        ...
        >
          ...
          >
            ...
            >
              <Select
                variant="filled"
                options={Object.entries(FILE_KIND_META).map(
                  ([value, meta]) => ({ value, label: meta.label }),
                )}
                onChange={(kind: WorkspaceFileKind) =>
                  form.setFieldValue('folder', kind)
                }
              />
            </Form.Item>
            <Form.Item
...
export default DataDevelopmentWorkbench;
